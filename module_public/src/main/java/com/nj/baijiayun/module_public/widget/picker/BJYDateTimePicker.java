 ...
 */
public class BJYDateTimePicker extends WheelPicker {
    /**
     * 不显示
     */
    ...
    private String yearSuffix, monthSuffix, daySuffix;
    private String hourSuffix, minuteSuffix;
     ...
     * @see #HOUR_12
     */
    public BJYDateTimePicker(Activity activity, @TimeMode int timeMode) {
        this(activity, YEAR_MONTH_DAY, timeMode);
    }

    public BJYDateTimePicker(Activity activity, @DateMode int dateMode, @TimeMode int timeMode) {
        super(activity);
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        selectedYearIndex = findItemIndex(years, year, yearSuffix);
        selectedMonthIndex = findItemIndex(months, month, monthSuffix);
        selectedDayIndex = findItemIndex(days, day, daySuffix);
        if (timeMode != NONE) {
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            selectedMonthIndex = findItemIndex(months, yearOrMonth, monthSuffix);
            selectedDayIndex = findItemIndex(days, monthOrDay, daySuffix);
        } else if (dateMode == YEAR_MONTH) {
            LogUtils.verbose(this, "change months while set selected");
            changeMonthData(yearOrMonth);
            selectedYearIndex = findItemIndex(years, yearOrMonth, yearSuffix);
            selectedMonthIndex = findItemIndex(months, monthOrDay, monthSuffix);
        }
        if (timeMode != NONE) {
            selectedHour = DateUtils.fillZero(hour);
            selectedMinute = DateUtils.fillZero(minute);
    ...
    public String getSelectedYear() {
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            ...
            return removeSuffix(years.get(selectedYearIndex), yearSuffix);
        }
        return "";
    }
    ...
    public String getSelectedMonth() {
        if (dateMode != NONE) {
            ...
            return removeSuffix(months.get(selectedMonthIndex), monthSuffix);
        }
        return "";
    }
    ...
    public String getSelectedDay() {
        if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
            ...
            return removeSuffix(days.get(selectedDayIndex), daySuffix);
        }
        return "";
    }
    ...
    protected View makeCenterView() {
        ...
        yearView.setDividerConfig(null);
        yearView.setTextColor(getContext().getResources().getColor(com.nj.baijiayun.module_common.R.color.common_main_text_color_content),
        ...
        monthView.setDividerConfig(null);
        monthView.setTextColor(getContext().getResources().getColor(com.nj.baijiayun.module_common.R.color.common_main_text_color_content),
                getContext().getResources().getColor(com.nj.baijiayun.module_common.R.color.common_main_text_color_title));
        final WheelView dayView = createWheelView();
        dayView.setDividerConfig(null);
        dayView.setTextColor(getContext().getResources().getColor(com.nj.baijiayun.module_common.R.color.common_main_text_color_content),
                getContext().getResources().getColor(com.nj.baijiayun.module_common.R.color.common_main_text_color_title));
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            ...
            yearView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    String selectedYearStr = removeSuffix(years.get(selectedYearIndex), yearSuffix);
                    ...
                    if (onWheelListener != null) {
                        onWheelListener.onMonthWheeled(selectedMonthIndex, removeSuffix(months.get(selectedMonthIndex), monthSuffix));
                    }
                    changeDayData(selectedYear, DateUtils.trimZero(removeSuffix(months.get(selectedMonthIndex), monthSuffix)));
                    dayView.setItems(days, selectedDayIndex);
                    if (onWheelListener != null) {
                        onWheelListener.onDayWheeled(selectedDayIndex, removeSuffix(days.get(selectedDayIndex), daySuffix));
                    }
                }
            });
        ...
        if (dateMode != NONE) {
            ...
            monthView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    String selectedMonthStr = removeSuffix(months.get(selectedMonthIndex), monthSuffix);
                    if (onWheelListener != null) {
    ...
    private int findItemIndex(ArrayList<String> items, int item, String suffix) {
        //折半查找有序元素的索引
        int index = Collections.binarySearch(items, String.valueOf(item) + suffix, new Comparator<Object>() {
            @Override
            public int compare(Object lhs, Object rhs) {
                String s = lhs.toString();
                String s1 = rhs.toString();
                String lhsStr = s.substring(0,s.length()-1);
                String rhsStr = s1.substring(0,s1.length()-1);
    ...
    private void initYearData() {
        ...
        if (startYear == endYear) {
            ...
            for (int i = startYear; i <= endYear; i++) {
                years.add(String.valueOf(i) + yearSuffix);
            }
        } else {
            //年份逆序
            for (int i = startYear; i >= endYear; i--) {
        ...
        if (!resetWhileWheel) {
            if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
                int index = years.indexOf(DateUtils.fillZero(Calendar.getInstance().get(Calendar.YEAR)) + yearSuffix);
                if (index == -1) {
    ...
    private void changeMonthData(int selectedYear) {
        ...
        if (!resetWhileWheel) {
            if (months.size() > selectedMonthIndex) {
                preSelectMonth = removeSuffix(months.get(selectedMonthIndex), monthSuffix);
            } else {
        ...
        if (startYear == endYear) {
            if (startMonth > endMonth) {
                for (int i = endMonth; i >= startMonth; i--) {
                    months.add(DateUtils.fillZero(i) + monthSuffix);
                }
            } else {
        ...
        if (!resetWhileWheel) {
            ...
            int preSelectMonthIndex = months.indexOf(preSelectMonth + monthSuffix);
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        ...
        if (!resetWhileWheel) {
            ...
            if (days.size() > selectedDayIndex) {
                //年或月变动时，保持之前选择的日不动
                preSelectDay = removeSuffix(days.get(selectedDayIndex), daySuffix);
            } else {
        ...
        if (selectedYear == startYear && selectedMonth == startMonth
            ...
            for (int i = startDay; i <= endDay; i++) {
                days.add(DateUtils.fillZero(i) + daySuffix);
            }
        ...
        if (!resetWhileWheel) {
            ...
            int preSelectDayIndex = days.indexOf(preSelectDay + daySuffix);
    ...
    private String removeSuffix(String str, String suffix) {
        return StringUtils.isEmpty(suffix) ? str : str.substring(0, str.indexOf(suffix));
    }
