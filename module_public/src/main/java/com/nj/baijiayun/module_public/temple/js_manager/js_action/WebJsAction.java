 ...
 * @describe
 */
public class WebJsAction implements IJsAction {
    @Override
    public void handlerData(Context context, AppWebView appWebView, JsActionDataBean jsActionDataBean) {
        if (jsActionDataBean.getParams().needShowAppTitle()) {
            JumpHelper.jumpWebView(jsActionDataBean.getUrl());
        } else {
            JumpHelper.jumpWebViewNoNeedAppTitle(jsActionDataBean.getUrl());
        }
    }
}
