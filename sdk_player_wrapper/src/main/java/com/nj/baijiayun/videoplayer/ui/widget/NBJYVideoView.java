...
public class NBJYVideoView extends BaseVideoView {
    ...
    public NBJYVideoView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (a.hasValue(R.styleable.BJVideoView_aspect_ratio)) {
            this.mAspectRatio = a.getInt(R.styleable.BJVideoView_aspect_ratio, AspectRatio.AspectRatio_16_9.ordinal());
        }

        if (a.hasValue(R.styleable.BJVideoView_render_type)) {
            this.mRenderType = a.getInt(R.styleable.BJVideoView_render_type, 1);
    ...
    public void updateAudioCoverStatus(boolean isAudio) {
        ...
        if (this.videoStatus != PlayerStatus.STATE_PLAYBACK_COMPLETED) {
            if (isAudio) {
                ...
                Glide.with(this).load(R.drawable.ic_audio_only).into(this.audioCoverIv);
            } else {
                this.audioCoverIv.setVisibility(8);
            }
