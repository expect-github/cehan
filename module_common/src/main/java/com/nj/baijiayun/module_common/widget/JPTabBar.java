...
import com.nj.baijiayun.module_common.widget.jptabbar.BadgeDismissListener;
import com.nj.baijiayun.module_common.widget.jptabbar.DensityUtils;
...
import com.nj.baijiayun.module_common.widget.jptabbar.TabException;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.Animatable;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.AnimationType;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.FlipAnimater;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.JumpAnimater;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.RotateAnimater;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.Scale2Animater;
import com.nj.baijiayun.module_common.widget.jptabbar.animate.ScaleAnimater;
import com.nj.baijiayun.module_common.widget.jptabbar.anno.NorIcons;
import com.nj.baijiayun.module_common.widget.jptabbar.anno.SeleIcons;
import com.nj.baijiayun.module_common.widget.jptabbar.anno.Titles;
...
public class JPTabBar extends LinearLayout implements ViewPager.OnPageChangeListener, View.OnTouchListener {
    ...
    private static final int DEFAULT_NO_TEXT_ICONSIZE = 28;
    //字体默认大小
    ...
    private int[] mNormalTextIcons;
    private int[] mSelectedTextIcons;
    ...
    private void initFromAttribute() {
        ...
        int iconNoTextSize = mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_TabIconNoTextSize, DensityUtils.dp2px(mContext, DEFAULT_NO_TEXT_ICONSIZE));
        ...
        if (!isInEditMode()) {
            ...
            for (int i = 0; i < mJPTabItems.length; i++) {
                ...
                mJPTabItems[i] = new JPTabItem.Builder(mContext).setTitle(mTitles == null ? null : mTitles[i]).setIndex(temp).setTextSize(textSize)
                        ...
                        .setBadgeTextSize(BadgetextSize).setSelectTextIcon(mSelectedTextIcons == null ? 0 : mSelectedTextIcons[i]).setNormalIcon(mNormalIcons[i]).setSelectedColor(selectColor).setBadgeHorMargin(badgeHorMargin)
                        .setBadgePadding(badgePadding).setIconSize(iconSize).setNotextIconSize(iconNoTextSize).setIconFilte(acceptFilter).setBadgeVerMargin(badgeVerMargin).setMargin(margin).setAnimater(animater)
                        .setSelectIcon(mSelectedIcons == null ? 0 : mSelectedIcons[i])
                        .setSelectedUrl(mSelectedRes==null?"":mSelectedRes[i])
                        .setNormalUrl(mNormalRes==null?"":mNormalRes[i])
                        .build();
    ...
    public JPTabBar setSelectedUrl(String... selectedUrl) {
        if (mSelectedRes == null) {
            this.mSelectedRes = selectedUrl;
        } else if (mSelectedRes.length == selectedUrl.length) {
            for (int i = 0; i < mSelectedRes.length; i++) {
                mJPTabItems[i].setSelectIconRemoteResources(selectedUrl[i]);
            }
            mSelectedRes = selectedUrl;
        } else {
            this.mSelectedRes = selectedUrl;
    ...
    public JPTabBar setNormalUrl(String... normalUrl) {
        if (mNormalRes == null) {
            this.mNormalRes = normalUrl;
        } else if (mNormalRes.length == normalUrl.length) {
            for (int i = 0; i < mNormalRes.length; i++) {
                mJPTabItems[i].setNormalIconRemoteResources(normalUrl[i]);
            }
            mNormalRes = normalUrl;
        } else {
            this.mNormalRes = normalUrl;
     ...
     * 设置为选中的字体图标数组
     */
    public JPTabBar setNormalTextIcons(int... normalIcons) {
        if (mNormalTextIcons == null) {
            this.mNormalTextIcons = normalIcons;
        } else if (mNormalTextIcons.length == normalIcons.length) {
            for (int i = 0; i < mNormalTextIcons.length; i++) {
                mJPTabItems[i].setNormalTextIcon(normalIcons[i]);
            }
            mNormalTextIcons = normalIcons;
        } else {
            this.mNormalTextIcons = normalIcons;
     ...
     * 设置选中字体图标
     */
    public JPTabBar setSelectedTextIcons(int... selectedIcons) {
        if (mSelectedTextIcons == null) {
            this.mSelectedTextIcons = selectedIcons;
        } else if (mSelectedTextIcons.length == selectedIcons.length) {
            for (int i = 0; i < mSelectedTextIcons.length; i++) {
                mJPTabItems[i].setSelectTextIcon(selectedIcons[i]);
                mJPTabItems[i].useIconAsSelect();
            }
            mSelectedTextIcons = selectedIcons;
        } else {
            this.mSelectedTextIcons = selectedIcons;
    ...
    public void setIconSize(int size) {
        if (mJPTabItems != null) {
            for (JPTabItem item : mJPTabItems) {
                ...
                item.getIconView().setLayoutParams(item.getIconView().getLayoutParams());
            }
        }
    }
